...
const entries = Array.from({ length: 20 }, (_, id) => {
  ...
  element.innerHTML = `<div class="agent-caption"><strong>Way ${id + 1}</strong><span>4 agents</span></div>`;
  grid.append(element);
  return { element, info: { id, agentId: id * 4, name: `Way ${id + 1}`, action: 'Moving', live: true, hidden: false } };
});
const shape = gridShape(20, grid.clientWidth, grid.clientHeight);
...
const send = () => {
  const scene: SceneSnapshot = { tick: tick++, guards: [], doors: [], cameras: [], keys: [], hole: false, power: true, truck: null, van: null,
    thieves: Array.from({ length: 80 }, (_, id) => {
      const cell = cells[id * 17 % cells.length];
      return { id, pose: { ...blankCharacterPose(), visible: true, scale: 1.28, moving: .7,
        phase: id * .65 + tick * .25, x: fineXYToWorldX(level, cell % level.w + .5), z: fineXYToWorldZ(level, Math.floor(cell / level.w) + .5), facingRad: tick * .015 + id } };
    }) };
...
const report = setInterval(() => {
  ...
  result.textContent = `Synthetic test: 80 agents → ${stats.feeds} ways · ${stats.cameraRenders} updates · max ${stats.maxPerFrame}/frame · CPU p95 ${stats.p95RenderMs.toFixed(1)}ms · ${((performance.now() - started) / 1000).toFixed(0)}s`;
