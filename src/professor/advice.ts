...
export const advice: Record<string, [string, string, string]> = {
  ...
  vault: ['Enter the vault', 'Use the card to cross the vault doorway. Entry completes automatically; the next step is opening a way out.', 'Walk through the vault door. No printing step is required.'],
  ...
  lockpick: ['Time your lockpick', 'Press Space when the moving marker enters the target zone. A miss attracts attention, so watch the rhythm.', 'Space attempts the current pin.'],
  wire: ['Cut the live wires', 'Look for the travelling sparks. Cut both live wires; avoid the earth wires, which cause a short and noise.', 'Up / down selects. Space cuts. Move sideways to leave.'],
