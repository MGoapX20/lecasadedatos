...
function copy(s: Snapshot, id: string): SceneCopy {
  if (id === 'recon')
    return {
      ...
      link: tr(
        s,
        'Walking around the base → mapping an organization’s public attack surface',
        'סיור סביב הבסיס ← מיפוי משטח התקיפה הציבורי של הארגון',
      ),
    };
  ...
  if (id === 'exfil')
    return {
      ...
      link: tr(
        s,
        'Drill → outbound channel · TOP SECRET documents → sensitive data · van → receiving server',
        'מקדחה ← ערוץ יוצא · מסמכים סודיים ביותר ← מידע רגיש · רכב מילוט ← שרת קולט',
      ),
    };
